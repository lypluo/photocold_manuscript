...
#added by YP: for align nonevent site years, align the data accoridng to green-up period
align_nonevents <- function( df,dovars,leng_threshold, before, after, nbins, do_norm=FALSE ){
  #!comment by YPL:test using some df
  # df<-df_nonevents.years
  # dovars<-c("vpd_day_fluxnet2015")
  # leng_threshold<-5
  # before=30
  # after=0
  # nbins=10
  ...
  #!comment by YPL:here changing the "is_event" to "greenup"
  if (!("greenup" %in% names(df))){   
    rlang::abort("align_nonevents(): Column named GS is missing in data frame df.")
  }
  ...
  ##--------------------------------------------------------
  ## Identify events ()-->here are the "greenup"
  ##--------------------------------------------------------
  greenups <- get_consecutive_greenup( 
    df$greenup, 
  ...
  if (nrow(greenups)>1){
    ...
    df_dday <- c()
    pos_greenup<-c()   ##added by YP-->the position of "greenups"
    
    for ( iinst in 1:nrow(greenups) ){
      #after_inst <- min( after, events$len[iinst] )
      after_inst<-greenups$len[iinst]  #revised by YP:selected the data until end of greenups
      dday <- seq( from=-before, to=after_inst, by=1 )
      idxs <- dday + greenups$idx_start[iinst]
      #
      pos_event<-c(pos_greenup,idxs)
  ...
  # tt_site<-df_dday[df_dday$sitename=="CA-Qfo",]
