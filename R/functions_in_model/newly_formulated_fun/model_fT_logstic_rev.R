...
f_Ts_logstic_rev<-function(df,
  ...
  #X0:lower temperature limit parameter(degreeC) above which f_Ts>0 and on the
     #state of acclimation(Tian et al., 2021)
  #b and Ts are the parameters for the sigmoid function of state of acclimation(S),
  #b-->is curvature of the sigmoid function and 
  #Ts is the inflection point of the sigmoid curve(the temperature at which
  #f_Ts reach half of the maximum(0.5))-->Gea-Izquierdo et al.,2010
  ...
  for (idx in seq(nrow(df))){
    ...
    f_stress_temp<-1/(1+exp((-1) *par["b"] * (S - par["Ts"])))
    #---------
