...
#update in 2022-07-->add an additional parameter m to adjust the p-model beyond the green-up period:
##-->the performance: the performance is very similar as the previous parameter setting(without parameter m)
#----------
library(tidyverse)
library(dplyr)
library(GenSA)
...
# save(par_PFTs,file = paste0("data/model_parameters/parameters_MAE_newfT_2pars/","optim_par_run5000_PFTs.rds"))

#--------------------------------------------------------------
#(5) compare the gpp_obs, ori modelled gpp, and gpp modelled using optimated parameters
#--------------------------------------------------------------
load(paste0("./data/model_parameters/parameters_MAE_newfT_2pars/","optim_par_run5000_PFTs.rds"))
...
for (i in 1:length(PFTs)) {
  ...
  scaling_factors <- df_sel %>%
    ...
    do({
      scaling_factor <- f_Ts_rev_2pars(.,par_PFTs[[i]])
      data.frame(
        sitename = .$sitename,
        date = .$date,
...
ggsave(paste0(save.path,"Figure5_pmodel_vs_obs_forClimPFTs_3set_parameter_fT_with2pars.png"),
