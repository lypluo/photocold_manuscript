...
mod_glmer1 <- lmer(lue ~ temp + log(vpd) + (1|sitename),
                    data=tmp)
# mod_glmer2 <- glmer(lue ~ temp + vpd + (1|sitename),
#                     data=tmp)
...
ddf <- ddf %>% 
  ...
  mutate(lue_glmer = predict(mod_glmer1, newdata = ., re.form = NULL)) %>% 
...
```{r, out.height="500%"}
df_meandoy %>% 
  pivot_longer(c(gpp_obs, gpp_pmodel, gpp_bess, gpp_rf, gpp_lue_const,gpp_temp_vpd,gpp_glmer), names_to = "model", values_to = "gpp") %>% 
  mutate(model = fct_relevel(model, "gpp_obs", "gpp_pmodel", "gpp_rf", "gpp_bess", "gpp_lue_const","gpp_temp_vpd","gpp_glmer")) %>% 
