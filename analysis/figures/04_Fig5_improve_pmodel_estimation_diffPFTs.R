...
cost_MF <- function(
  ...
  #tidy data numbers for each site(N);weigth for each site(w_factor=N/total obs) 
  t_info<-data%>%
    group_by(sitename)%>%
    summarise(N=length(gpp))%>%
    mutate(Ntotal= sum(N),w_factor=N/Ntotal)
  data<-left_join(data,t_info)
  ...
  mse_allsites<-df%>%
    select(sitename,gpp,gpp_mod,scaling_factor,w_factor)%>%
    mutate(bias_2=c(gpp-gpp_mod*scaling_factor)^2)%>%
    group_by(sitename)%>%
    summarise(mse=mean(bias_2,na.rm=T),w_factor=mean(w_factor))%>%
    mutate(mse_w=mse*1/w_factor) #taking considering the numbers of obs in each site
  mse<-mean(mse_allsites$mse_w)
  #mae:mean absolute error:
  # mae<-sum(abs(df$gpp - df$gpp_mod * df$scaling_factor))/nrow(df)
  # This visualizes the process,
  # comment out when running for real
...
##
df_merge_new<-left_join(df_merge_new,df_phenos,by=c("sitename","year"))
#
df_meandoy_Clim_PFTs<-df_merge_new %>%
  ...
  mutate(gpp_bias_model_ori=gpp_mod_old_ori - gpp_obs,
         gpp_bias_model_optim=gpp_mod_recent_optim - gpp_obs)%>%
  ...
  dplyr::summarise(gpp_obs_min=min(gpp_obs,na.rm = T),gpp_obs_max=max(gpp_obs,na.rm = T),
                   ...
                   mod_recent_ori=mean(gpp_mod_recent_ori, na.rm = TRUE),
                   mod_recent_optim=mean(gpp_mod_recent_optim,na.rm = TRUE),
                   mod_bias_ori=mean(gpp_bias_model_ori,na.rm=T),
                   mod_bias_optim=mean(gpp_bias_model_optim,na.rm=T)
                   )%>%
  #add in May,2023
...
gpp_mean_bias<-df_meandoy_Clim_PFTs%>%
  filter(avg_period==TRUE)%>%
  group_by(Clim_PFTs)%>%
  dplyr::summarise(gpp_bias_ori_greenup=mean(mod_bias_ori,na.rm=T),
                   gpp_bias_optim_greenup=mean(mod_bias_optim,na.rm=T))
#merge gpp_mean_bias to datasets:
df_meandoy_Clim_PFTs<-left_join(df_meandoy_Clim_PFTs,
                                     gpp_mean_bias)

#for Clim-PFTs
season_plot<-df_meandoy_Clim_PFTs %>%
  group_by(Clim_PFTs, doy) %>%
  #add gpp_obs = gpp_obs_mean-->to make the plots
  mutate(gpp_obs=gpp_obs_mean)%>%
  pivot_longer(c(gpp_obs,mod_old_ori,mod_recent_optim), names_to = "Source", values_to = "gpp") %>%
  ggplot() +
  geom_ribbon(aes(x=doy,ymin=gpp_obs_mean - gpp_obs_sd,
                  ymax=gpp_obs_mean + gpp_obs_sd),fill="black",alpha=0.3)+
  geom_line(aes(doy, gpp, color = Source),size=0.8) +
  #adding the gpp bias-->May,2023:
  geom_segment(aes(x=120,y=0,xend=120+gpp_bias_ori_greenup*50,yend=0),col="orange",size=1.1)+
  geom_segment(aes(x=120,y=-1,xend=120+gpp_bias_optim_greenup*50,yend=-1),col="dodgerblue",size=1.1)+
  geom_segment(aes(x=120,y=-1.8,xend=120,yend=0.8),col=adjustcolor("black",0.8),size=1.02)+
  #adding the bias value:
  geom_text(aes(x=160,y=1,label=round(gpp_bias_ori_greenup,2)))+
  geom_text(aes(x=160,y=-2,label=round(gpp_bias_optim_greenup,2)))+
  labs(y = expression( paste("GPP (g C m"^-2, " d"^-1, ")" ) ),
       x = "DoY") +
  scale_color_manual("GPP sources",values = c("mod_old_ori" = "orange",
  "mod_recent_optim" = "dodgerblue", "gpp_obs" = "black"),
  # labels = c("Orig. P-model", "Cali. P-model","Observations")) +
  labels=c(expression(GPP[Pmodel]),expression(GPP[adj]),expression(GPP[obs])))+  ##updated in Nov,2022
  labs(y = expression( paste("GPP (g C m"^-2, " d"^-1, ")" ) ),
       x = "DoY") +
