...
#--------------------
#1)for all the sites:
plot_para_each<-function(df,para,do_legend){
  # df<-plot_data
  # para<-"a2"
  # do_legend=FALSE
  
  df<-df %>%
    select(sitename,tmin,PFT,para)
...
##
p_tmin_a1<-plot_para_each(plot_data,"a1",TRUE)
p_tmin_a2<-plot_para_each(plot_data,"a2",FALSE)
p_tmin_b1<-plot_para_each(plot_data,"b1",FALSE)
p_tmin_b2<-plot_para_each(plot_data,"b2",FALSE)
p_tmin_k<-plot_para_each(plot_data,"k",FALSE)

#merge the plots:
paras_general_range<-cowplot::plot_grid(p_tmin_a1,p_tmin_a2,
      p_tmin_b1,p_tmin_b2,
      p_tmin_k,
      nrow=3, ncol = 2,labels = "auto",label_size = 20,align = "hv")
#save the plot
save.path<-"./manuscript/test_files/check_paras_in_Beni_method/"
ggsave(paste0(save.path,"Check_paras_general_beni_method.png"),
...
plot_paras<-function(df,Env_var,para,do_legend){
  ...
  # para<-"b2"
  # do_legend=FALSE
  # for example: Tmean vs a2
  #I.site-level
  df_site_level<-df %>%
    dplyr::select(sitename,PFT,koeppen_code,Clim.PFTs,para,tmin,temp)
...
p_tmin_a1<-plot_paras(df = plot_data,Env_var = "tmin",
                       para = "a1",TRUE)
p_tmin_a2<-plot_paras(df = plot_data,Env_var = "tmin",
                      para = "a2",FALSE)  
p_tmin_b1<-plot_paras(df = plot_data,Env_var = "tmin",
                      para = "b1",FALSE)  
p_tmin_b2<-plot_paras(df = plot_data,Env_var = "tmin",
                      para = "b2",FALSE)  
p_tmin_k<-plot_paras(df = plot_data,Env_var = "tmin",
                        para = "k",FALSE)
...
#save the plot
save.path<-"./manuscript/test_files/check_paras_in_Beni_method/"
ggsave(paste0(save.path,"Check_paras_beni_method.png"),
