...
df_meandoy_Clim_PFTs_lue <- ddf_lue %>%
  ...
  #update in Jul,2023:convert the LUE unit:ppdf-->W m-2: 1 W m-2 = 2.114 ppfd
  mutate(across(starts_with("lue"),~ .x*2.114))%>%
...
plot_final_lue_1 <- df_meandoy_Clim_PFTs_lue %>% 
  ...
  #      x = "DoY") +
  #update in Jul,2023-->LUE unit: g C W-1 d-1
  labs(y = expression( paste("LUE (g C W"^-1*" d"^-1, ")" ) ),
       x = "DoY") +
