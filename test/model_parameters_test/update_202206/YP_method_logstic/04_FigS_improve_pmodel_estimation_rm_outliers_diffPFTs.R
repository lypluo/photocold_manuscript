...
source(paste0("./R/functions_in_model/newly_formulated_fun/model_fT_logstic_rev.R"))
#--------------------------------------------------------------
#(2) retreive the optimized parameter for the selected sites
#--------------------------------------------------------------
# set initial value
# set initial value
par=c("tau"=5,"X0"=-10,"b"=0.2,"Ts"=5,"k"=1)
#
lower=c(1,-10,0,0,0)
upper=c(25,10,1,25,2)
...
cost <- function(
  ...
  scaling_factor <- data %>%
    # group_by(sitename) %>%
    do({
      scaling_factor <- f_Ts_logstic_rev(
        .,
        par
      )
...
# save(par_PFTs,file = paste0("data/model_parameters/parameters_MAE_newfT_logistic/","optim_par_run5000_PFTs.rds"))

#--------------------------------------------------------------
#(5) compare the gpp_obs, ori modelled gpp, and gpp modelled using optimated parameters
#--------------------------------------------------------------
load(paste0("./data/model_parameters/parameters_MAE_newfT_logistic/","optim_par_run5000_PFTs.rds"))
...
for (i in 1:length(PFTs)) {
  ...
  scaling_factors <- df_sel %>%
    ...
    do({
      scaling_factor <- f_Ts_logstic_rev(.,par_PFTs[[i]])
      data.frame(
        sitename = .$sitename,
        date = .$date,
...
ggsave(paste0(save.path,"FigureS_pmodel_vs_obs_forPFTs_Mekela2008_logic.png"),season_plot,width = 15,height = 10)
...
ggsave(paste0(save.path,"Figure5_pmodel_vs_obs_forClimPFTs_3set_parameter_fT_logistic.png"),
